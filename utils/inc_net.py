            ...
            if nextperiod_initialization is not None:
                ...
                weight = torch.cat(
                    [
                        weight,
                        torch.zeros(
                            nb_classes - nb_output, self.feature_dim
                        ).cuda(),  # 补上新类的
                    ]
                )
            fc.weight = nn.Parameter(weight)
